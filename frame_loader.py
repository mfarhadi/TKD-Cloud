...
def Frame_reader(info):
    ...
    for path, img, im0s, vid_cap in dataset:
      ...
      info.frame[0,:,0:img.shape[1],:] = torch.from_numpy(img).to(info.device)
      info.collecting=False
      while not info.ready:
          time.sleep(0.0005)
      info.collecting = True
      info.ready = False
      info.results=info.results.unsqueeze(0)

      for i, det in enumerate(non_max_suppression(info.results, info.opt.conf_thres, info.opt.nms_thres)):  # detections per image
          s,im0='',im0s
